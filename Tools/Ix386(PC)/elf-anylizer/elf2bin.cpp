...
int main(int argc, char * argv[])
{
  if( argc != 3 )
  {
   cout << "Usage : elf2bin <elfFile> <binFile>\n";
   return -1;
  }

  ifstream elfFile( argv[1], ios::in | ios::binary );
  if( elfFile.bad() )
  {
   cout << "\nCan't find or open file " << argv[2] << ".\n";
   return 1;
  }

  ofstream binFile( argv[2], ios::out | ios::binary );
  ELFHeader hd;
  ...
  for(int i = 0; i < ph.getEntryCount(); i++)
  {
    if( (*p[i]).p_type == 1 )
    {
      cout << "virt address: " << hex << (*p[i]).p_vaddr << endl;
      elfFile.seekg(0);
