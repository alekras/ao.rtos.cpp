...
/*---------------------- KeyboardISAO:: active object -------------------------*/

KeyboardISAO::KeyboardISAO(DWORD prio, Display *dspl) : ISAObject( prio, 1 ), display(dspl) {
  PC_VectSet( 0x21, keyboardISR );
...
DWORD
KeyboardISAO::processMessage( Message *e ) {
  BYTE scanCode, direction = 1;
  switch( e->getMessageID() ) {
    case kbd :              // Event from Keyboard
     scanCode = (BYTE) e->getBinaryData();
     switch( scanCode ) {
       case 0xE0 :
         kbdFlags3.bit.E0 = 1;
         kbdFlags3.bit.E1 = 0;
       ...
       case 0xE1 :
         ...
         direction = 2;
         break;
       case 0x53 :  // Delete scan code
         if( (kbdFlags1.bit.Ctrl == 1) && (kbdFlags1.bit.Alt == 1) ) {
           // REBOOT : do nothing.
           direction = 2;
         } else {
           direction = 0;
         ...
         break;
       case 0x52 :
         kbdFlags2.bit.Insert = 1;
         direction = 0;
         break;
       case 0x52 + 0x80 :
         if (kbdFlags2.bit.Insert == 0) break;
         kbdFlags2.bit.Insert = 0;
         kbdFlags1.bit.Insert ^= 1;
         break;

       case 0x2a :
         kbdFlags1.bit.LShift = 1;
         break;
       case 0x2a + 0x80 :
       	 kbdFlags1.bit.LShift = 0;
         break;

       case 0x36 :
         kbdFlags1.bit.RShift = 1;
         break;
       case 0x36 + 0x80 :
       	 kbdFlags1.bit.RShift = 0;
         break;

       case 0x38 : // ALt scan code
         kbdFlags1.bit.Alt = 1;
         break;
       case 0x38 + 0x80 :
         kbdFlags1.bit.Alt = 0;
       ...
       case 0x1D : // Control scan code
         kbdFlags1.bit.Ctrl = 1;
         break;
       case 0x1D + 0x80 :
         kbdFlags1.bit.Ctrl = 0;
         break;

       case 0x3a : // CapsLock scan code
         if (kbdFlags2.bit.CapsLock == 1) break;
         kbdFlags2.bit.CapsLock = 1;
         kbdFlags1.bit.CapsLock ^= 1;
         break;
       case 0x3a + 0x80 :
         kbdFlags2.bit.CapsLock = 0;
         setLEDs();
         break;

       case 0x46 : // ScrollLock scan code
         if (kbdFlags2.bit.Scroll == 1) break;
         kbdFlags2.bit.Scroll = 1;
         kbdFlags1.bit.Scroll ^= 1;
         break;
       case 0x46 + 0x80 :
         kbdFlags2.bit.Scroll = 0;
         setLEDs();
         break;

       case 0x45 : // Numlock scan code
         if (kbdFlags2.bit.NumLock == 1) break;
         kbdFlags2.bit.NumLock = 1;
         kbdFlags1.bit.NumLock ^= 1;
         break;
       case 0x45 + 0x80 :
         kbdFlags2.bit.NumLock = 0;
         setLEDs();
         break;
       default:
      	 direction = 0;
     ...
     switch( direction ) {
       case 0 :
         if( (scanCode & 0x80)  == 0 ) {
           DWORD ascii = convert( scanCode );
           if( ascii != 0 ) {
             Message e(0, 0, ascii, show );
             putOutgoingMessage( &e );
//             display->setPosition( 0, 14 + (count % 4) );
//             display->printf("  %4h  =  '%2h' [%d]", ascii, scanCode, count++);
           }
         }
       case 1 :
         kbdFlags3.bit.E0 = 0;
...
AO_STACK *
KeyboardISAO::serviceInterrupt( AO_STACK * stp ) {
  WORD_S count = 0;
  BYTE kbdStatus = 0, data, skip = 0;
  ...
  if( data == 0xEE ) {
	...
	if( data == 0xFA ) {
    kbdFlags4.bit.Acknowledge = 1;
    skip++;
  }
  if( data == 0xFE ) {
    kbdFlags4.bit.Resend = 1;
    skip++;
  }
  if( skip == 0 ) {
    Message e(0, 0, data, kbd);
    putOutgoingMessage( &e );
  }
...
void
KeyboardISAO::sendCommand( BYTE cmd ) {
  WORD_S count;
  BYTE kbdStatus;
  BYTE_S retry = 3;

  do {
    kbdFlags4.bit.Acknowledge = 0;
    kbdFlags4.bit.Resend = 0;
    kbdFlags4.bit.Error = 0;
  	count = 0;
    while( --count < 0 ) {
      inp( 0x64, kbdStatus );
      if( (kbdStatus & 0x02) == 0 )
    ...
    count = 0;
    while( --count < 0 ) {
      if( kbdFlags4.bit.Acknowledge == 1 ) {
        kbdStatus = 0;
        break;
      }
    }
    if(--retry < 0) {
      kbdFlags4.bit.Error = 1;
      break;
    }
  } while( kbdStatus == 1 );
}
...
void
KeyboardISAO::setLEDs() {
	BYTE b = kbdFlags1.byte;
	b = (BYTE) ((b >> 4) & 0x07);
	kbdFlags4.bit.CapsLockLED = kbdFlags1.bit.CapsLock;
	kbdFlags4.bit.NumLockLED = kbdFlags1.bit.NumLock;
	kbdFlags4.bit.ScrollLED = kbdFlags1.bit.Scroll;
	command(0xAD);
	sendCommand(0xED);
	sendCommand(b);
	command(0xAE);
...
WORD
KeyboardISAO::convert( BYTE scanCode ) {
	WORD row = (WORD) scanCode;
	BYTE isShift = (kbdFlags1.bit.RShift == 1) || (kbdFlags1.bit.LShift == 1);
	row *= 8;
	if( kbdFlags1.bit.Alt == 1 ) {
		row += 3;
	}
	else if( kbdFlags1.bit.Ctrl == 1 ) {
		row += 2;
	} else if( scanCode >= 0x47 ) {
		if( kbdFlags1.bit.NumLock == 1 ) {
			if( isShift ) {
				row += 7;
			} else {
				row += 4;
	...
	} else if( kbdFlags1.bit.CapsLock == 1 ) {
		if( isShift ) {
			row += 6;
		} else {
			row += 5;
